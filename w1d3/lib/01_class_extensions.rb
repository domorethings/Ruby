...
class String
  def caesar(shift)
    ...
    string.each_byte do |ascii|
      ...
      # use module to account for overshifts
      shifted_position = (letter_position + shift) % 26

      # add back "a".ord to shifted_position and send to encoded_string
      encoded_string << (shifted_position + "a".ord).chr
    end
...
class Hash
  def difference(other_hash)
    # subtracts the difference between two hashes
    difference_hash = {} # stores the difference between two hashes

    # iterate through the keys
    self.keys.each do |key|
      # do not add keys that are included in the other_hash
      difference_hash[key] = self[key] unless other_hash.key?(key)
    end 

    # iterate through other_hash's key. avoid duplicates
