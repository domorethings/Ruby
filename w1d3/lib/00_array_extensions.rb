...
class Array
  def square!
    # mutate the original array to get the squares of each element
    self.map! do |el|
      el = el * el  
    end
  end
...
class Array
  def my_uniq
    result = []
    # return the unique elements in order of appearance
    self.each do |el|
...
class Array
  # returns the positions in the array where the sum of 
  # zero is found 
   def two_sum
    pairs = []

    # iterate through the array according to the size of the current array
    self.count.times do |i1|
      ((i1+1)..(self.count-1)).each do |i2|
        # send the currnt positions to
...
class Array
  def median
    ...
    sorted = self.sort
    # even number return the average of the middle two numbers
    if self.length.even?
      median = (sorted[length / 2] + sorted[length / 2 - 1]).fdiv(2)
    # odd number, return the middle number
